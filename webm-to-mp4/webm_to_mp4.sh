...
function convert {
    ...
    if [ -n "$assume_yes" ]; then
        ffmpeg -vsync vfr -i "$file" -c:v libx264 -c:a aac -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" "$filename.mp4" -y
    else
        ffmpeg -vsync vfr -i "$file" -c:v libx264 -c:a aac -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" "$filename.mp4"
    fi
